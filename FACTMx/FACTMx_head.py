...
class FACTMx_head_Multinomial2(FACTMx_head):
  head_type = 'Multinomial2'
  ...
  def from_config(config):
    return FACTMx_head_Multinomial2(**config)
...
class FACTMx_head_MultiNormal(FACTMx_head):
  ...
  def loss(self, data, latent, beta=1):
    ...
    loss = -tf.reduce_mean(log_prob)
    #loss += (_data_dim ** 2) * tf.reduce_sum(tf.math.log(scale)) / _batch_size
