...
class FACTMx_head_ZINB_hierarchy(FACTMx_head):
  head_type = 'ZINB_hierarchy'
  ...
  def get_config(self):
    config = {
        ...
        'mixture_params_list':[
          ...
          } for logits, log_total_count  in zip(self.level_logits, self.level_log_total_count)
        ],
    }
    return config

  def from_config(config):
    return FACTMx_head_ZINB_hierarchy(**config)
...
#     if level == 0:
#       logits = self.level_logits[0]
#       log_total_count = self.level_log_total_count[0]
...
#         inflated_loc_probs=tf.reshape(inflated_loc_probs, _flat_shape),
#         require_integer_total_count=False,
#     ), [logits, log_total_count, inflated_loc_logits]
