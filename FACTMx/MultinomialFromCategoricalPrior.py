...
class MultinomialFromCategoricalPrior(FACTMx_head):
  ...
  def __init__(
    ...
    if prior_locs is None:
      prior_locs = tf.keras.initializers.Orthogonal((self.dim_latent, self.dim))()
      prior_locs *= tf.math.sqrt(self.dim)
    self.prior_locs = tf.constant(prior_locs, dtype='float32')

    if prior_scales is None:
      prior_scales = tf.eye(self.dim, batch_shape=[self.dim_latent])
  ...
  def loss(self,
    ...
    locs = tf.expand_dims(self.prior_locs, axis=0)
    scales = tf.expand_dims(self.prior_locs, axis=0)
    decoder_prob = tfp.distributions.MultivariateNormalDiag(locs, scales).prob(skip_connection)
    decoder_prob = tf.reduce_sum(decoder_prob * latent, axis=-1)
    decoder_log_prob = tf.math.log(decoder_prob)
    encoder_log_prob = tfp.distributions.MultivariateNormalDiag(preencoded, preencoded_scale).log_prob(skip_connection)

    return loss - decoder_log_prob + encoder_log_prob
