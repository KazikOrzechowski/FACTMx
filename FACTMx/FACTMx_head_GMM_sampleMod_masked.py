...
class FACTMx_head_GMM_sampleMod_masked(FACTMx_head):
  head_type = 'GMM_sampleMod_masked'

  def __init__(self,
               dim, dim_latent, dim_normal, dim_covariates,
               head_name,
               decode_mixture_config='linear',
               encoder_classifier_config='linear',
               mixture_params={'loc': 'random', 
                               'log_cov_diag': 0., 
                               'cov_perturb_factor': 'zeros',
                               'linear_mod': 'random'},
               temperature=1E-4, 
               eps=1E-3, 
    ...
    linear_modification = mixture_params.pop('linear_mod', 'random')
    if linear_modification == 'random':
      linear_modification = tf.keras.initializers.RandomNormal()(shape=(dim_covariates+1, dim_normal))
  ...
  def get_modified_mixtures(self, data):
    covariates = data[1]
    modification = tf.concat([covariates, tf.ones(covariates.shape[0])]) @ self.linear_mixture_modification

    locs = tf.reshape(self.mixture_locs, (1, 1, self.dim+1, self.dim_normal)) +\
           tf.reshape(modification, (-1, 1, 1, self.dim_normal))
  ...
  def decode(self, latent, data):
    gmm_data, covariates = data
    log_mixture_probs = self.decode_log_mixture_probs(latent)
    log_mixture_probs = tf.reshape(log_mixture_probs,
                                    (-1, 1, self.dim+1))

    mixtures = self.get_modified_mixtures(data)

    _broadcastable_shape = (-1, tf.shape(gmm_data)[1], 1, self.dim_normal)
    reshaped_data = tf.reshape(gmm_data, _broadcastable_shape)
  ...
  def encode(self, data):
    gmm_data, covariates = data
    assignment_logits = self.encoder_classifier(gmm_data)
    assignment_sample = self.get_assignment_distribution(assignment_logits).sample()
  ...
  def from_config(config):
    return FACTMx_head_GMM_sampleMod_masked(**config)
