"""
Synthetic-data helpers for exercising FACTMx models.

These functions are not pytest-style assertions.  Each helper builds a model
configuration, a TensorFlow dataset, optional validation data, and metadata for
a small simulated problem.  They are useful for demos, smoke tests, and manual
...
def test_Normal_2D(sd1, sd2, covar=0., beta=1):
  """Return a two-dimensional centered Normal synthetic problem."""
  data_loc = tf.zeros((2,))
...
def test_Normal_2D_bias(sd1, sd2, covar=0., beta=1):
  """Return a two-dimensional Normal synthetic problem with mean one."""
  data_loc = tf.ones((2,))
  data_scale = tfp.math.fill_triangular([sd2, covar, sd1])
...
def test_Normal_3D(sd1, sd2, covar=0., dim_latent=1, beta=1):
  """Return a three-dimensional Normal synthetic problem."""
  data_loc = tf.zeros((3,))
...
def test_Normal_ND_with_noise(sd, n_dim, sd_noise=0.1, noise_dim=10, dim_latent=1, beta=1):
  """Return a Normal synthetic problem with signal and noise dimensions."""
...
def test_N_Bernoullis(n, scale=1, beta=1):
  """Return multiple Bernoulli views generated from one latent dimension."""
...
def test_Bernoulli_Normal(n, beta=1):
  """Return a mixed Bernoulli-and-Normal synthetic problem."""
...
def test_combined(dim_latent, beta=1):
  """Return a combined multinomial-and-Normal synthetic problem."""
  latent_loc = tf.zeros((2,))
  latent_scale = tf.eye(2)
...
def test_Multinomial(dim_latent, beta=1):
  """Return a multinomial synthetic problem with latent-generated logits."""
  latent_loc = tf.zeros((2,))
  latent_scale = tf.eye(2)
...
def test_Topic(dim_latent, beta=1, n_patients=100, n_obs=1000, flexTopic=True):
  """Return a topic-model synthetic problem with sampled topic assignments."""
  latent_loc = tf.zeros((2,))
  latent_scale = tf.eye(2)
...
def test_Normal_noise_regularisation(sd, n_dim,
  ...
  """Return a Normal synthetic problem with configurable noise regularisation."""
  data_loc = tf.zeros((n_dim + noise_dim,))
  data_scale = tf.linalg.diag([sd]*n_dim + [sd_noise]*noise_dim)
