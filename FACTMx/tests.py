...
def test_Normal_2D(sd1: float, sd2: float, covar: float = 0., beta: float = 1) -> tuple[dict[str, Any], tf.data.Dataset, list[Any], dict[str, Any]]:
  data_loc = tf.zeros((2,))
...
def test_Normal_2D_bias(sd1: float, sd2: float, covar: float = 0., beta: float = 1) -> tuple[dict[str, Any], tf.data.Dataset, list[Any], dict[str, Any]]:
  data_loc = tf.ones((2,))
  data_scale = tfp.math.fill_triangular([sd2, covar, sd1])
...
def test_Normal_3D(sd1: float, sd2: float, covar: float = 0., dim_latent: int = 1, beta: float = 1) -> tuple[dict[str, Any], tf.data.Dataset, list[Any], dict[str, Any]]:
  data_loc = tf.zeros((3,))
...
def test_Normal_ND_with_noise(sd: float, n_dim: int, sd_noise: float = 0.1, noise_dim: int = 10, dim_latent: int = 1, beta: float = 1) -> tuple[dict[str, Any], tf.data.Dataset, list[Any], dict[str, Any]]:
...
def test_N_Bernoullis(n: int, scale: float = 1, beta: float = 1) -> tuple[dict[str, Any], tf.data.Dataset, list[Any], dict[str, Any]]:
  ...
  data_bernoulli = [tf.cast(view, dtype='float32') for view in data_bernoulli]
...
def test_Bernoulli_Normal(n: int, beta: float = 1) -> tuple[dict[str, Any], tf.data.Dataset, list[Any], dict[str, Any]]:
...
def test_combined(dim_latent: int, beta: float = 1) -> tuple[dict[str, Any], tf.data.Dataset, list[Any], dict[str, Any]]:
  latent_loc = tf.zeros((2,))
  latent_scale = tf.eye(2)
...
def test_Multinomial(dim_latent: int, beta: float = 1) -> tuple[dict[str, Any], tf.data.Dataset, list[Any], dict[str, Any]]:
  latent_loc = tf.zeros((2,))
  latent_scale = tf.eye(2)
...
def test_Topic(dim_latent: int, beta: float = 1, n_patients: int = 100, n_obs: int = 1000, flexTopic: bool = True) -> tuple[dict[str, Any], tf.data.Dataset, list[Any], dict[str, Any]]:
  latent_loc = tf.zeros((2,))
  latent_scale = tf.eye(2)
  ...
  if flexTopic:
    heads_config = [{'head_type':'Topic', 'dim':3,
...
def test_Normal_noise_regularisation(sd: float, n_dim: int,
                                     sd_noise: float = 0.1, noise_dim: int = 10,
                                     dim_latent: int = 1, beta: float = 1,
                                     l1: float = 0.1, l2: float = 0.1) -> tuple[dict[str, Any], tf.data.Dataset, list[Any], dict[str, Any]]:
